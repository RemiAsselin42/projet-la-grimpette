...
try {
    ...
    $stmt = $bdd->prepare("
            SELECT id_activite, nom_activite, date, description, heure, categorie FROM activite
        ");
    $stmt->execute();
    ...
    $jsonFile = $jsonDir . "/activites.json";
    ...
    // Écrire le fichier
    if (file_put_contents($jsonFile, json_encode($allActivites, JSON_PRETTY_PRINT))) {
